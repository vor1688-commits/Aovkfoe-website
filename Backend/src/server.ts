...
//             statusFilterClause = `AND b.status = $${queryParams.length + 1}`;
//             queryParams.push(status as string);
//         }

//         const summaryQuery = `
//             WITH filtered_bills AS (
//                 SELECT b.id, b.total_amount FROM bills b JOIN users u ON b.user_id = u.id
//                 WHERE b.created_at BETWEEN $1 AND $2 ${userFilterClause} ${statusFilterClause}
//             ),
//             winning_items AS (
//                 SELECT
//                     be.bill_id,
...
//                 JOIN bills b ON be.bill_id = b.id
//                 JOIN lotto_rounds lr ON b.lotto_round_id = lr.id
//                 WHERE be.bill_id IN (SELECT id FROM filtered_bills)
...
//             )
//             SELECT
//                 (SELECT COALESCE(SUM(total_amount), 0) FROM filtered_bills)::float AS "totalBetAmount",
...
//                 (SELECT COUNT(DISTINCT bill_id) FROM winning_items) AS "winningBills"
//         `;
...
//         `;
//         const mostBetNumbersQuery = `
//             SELECT bi.bet_number as "number", COUNT(bi.id) as "count", SUM(bi.price)::float as "totalAmount"
//             FROM bet_items bi JOIN bill_entries be ON bi.bill_entry_id = be.id JOIN bills b ON be.bill_id = b.id JOIN users u ON b.user_id = u.id
//             WHERE b.created_at BETWEEN $1 AND $2 ${userFilterClause} ${statusFilterClause}
//             GROUP BY bi.bet_number ORDER BY "count" DESC, "totalAmount" DESC LIMIT 10;
//         `;
//         const recentBillsQuery = `
//             SELECT 
//                 b.id, b.bill_ref AS "billRef", u.username, 
//                 b.created_at AS "createdAt", b.total_amount AS "totalAmount", 
//                 b.status, b.bet_name AS "lottoName", b.bill_lotto_draw AS "billLottoDraw", b.note
//             FROM bills b JOIN users u ON b.user_id = u.id
//             WHERE b.created_at BETWEEN $1 AND $2 ${userFilterClause} ${statusFilterClause}
...
//             SELECT
//                 CASE
//                     WHEN lt.name LIKE '%ไทย%' THEN 'ไทย'
//                     WHEN lt.name LIKE '%ลาว%' THEN 'ลาว'
//                     WHEN lt.name LIKE '%ฮานอย%' THEN 'เวียดนาม'
//                     WHEN lt.name LIKE '%มาเลย์%' THEN 'มาเลเซีย'
//                     WHEN lt.name LIKE '%หุ้น%' THEN 'หุ้น'
//                     ELSE 'อื่นๆ'
//                 END AS country,
...
//         const [
//             summaryResult, byLottoTypeResult, mostBetNumbersResult, 
//             recentBillsResult, byCountryResult
//         ] = await Promise.all([
//             client.query(summaryQuery, queryParams),
//             client.query(byLottoTypeQuery, queryParams),
//             client.query(mostBetNumbersQuery, queryParams),
//             client.query(recentBillsQuery, queryParams),
//             client.query(byCountryQuery, queryParams)
//         ]);
...
//             mostBetNumbers: mostBetNumbersResult.rows,
...
// เพิ่มเข้าไปใน src/server.ts


// ใน server.ts, แทนที่ API เดิม
...
//         // ⭐ Logic การสร้างเงื่อนไข WHERE ที่แก้ไขใหม่
//         // 1. ให้ความสำคัญกับ lottoDate ก่อน
...
//         } else {
//             // ถ้าไม่เลือก lottoDate ให้ใช้ startDate, endDate แทน
...
//         // --- Queries (ส่วนนี้จะทำงานถูกต้องตามเงื่อนไขใหม่) ---
//         const summaryQuery = `
...
//         const allBetItemsSummaryQuery = `SELECT bi.bet_number as "number", SUM(bi.price)::float as "totalAmount", COUNT(bi.id) as "count" FROM bet_items bi JOIN bill_entries be ON bi.bill_entry_id = be.id ${baseFromJoin.replace('FROM bills b', 'JOIN bills b ON be.bill_id = b.id')} WHERE ${baseWhereClauses} AND b.status = 'ยืนยันแล้ว' GROUP BY bi.bet_number ORDER BY "totalAmount" DESC;`;
