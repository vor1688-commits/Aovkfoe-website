...
app.post("/api/savebills", async (req: Request, res: Response) => {
    ...
    try {
        ...
        const { 
            ...
            closed_numbers: closedNumbers, // เลขปิดจากฐานข้อมูล
        ...
        for (const entry of billEntries) {
            ...
            const processBetItems = async (originalPrice: number, style: string, standardRate: number) => {
                ...
                for (const betNumber of entry.bets) {
                    const isClosed = closedNumbers.includes(betNumber);

                    if (isClosed) {
                        // ถ้าเป็นเลขปิด: บันทึกทุกอย่างเป็น 0 และสถานะเป็น 'คืนเลข'
                        await client.query(
                            `INSERT INTO bet_items (bill_entry_id, bet_number, price, bet_style, rate, payout_amount, baht_per, status) 
                             VALUES ($1, $2, 0, $3, 0, 0, 0, 'คืนเลข')`,
                            [newBillEntryId, betNumber, style]
                        );
                    } else {
                        // ถ้าไม่ใช่เลขปิด: ใช้ Logic เดิมในการคำนวณเลขจ่ายครึ่ง
