...
app.post("/api/savebills", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const allBetNumbers = [...new Set(billEntries.flatMap((entry) => entry.bets))];
        if (allBetNumbers.length > 0) { // ตรวจสอบต่อเมื่อมีเลขที่ต้องเช็ค
            const [rangeLimitsResult, spentResult, roundLimitsResult] = yield Promise.all([
            ...
            const getMostSpecificRule = (rules, type) => {
                const filteredRules = rules.filter(r => type.includes(r.number_limit_types));
            ...
            for (const betNumber in incomingTotals) {
                ...
                const applicableRules = rangeLimits.filter(r => r.range_start && r.range_end &&
                ...
                if (applicableRules.length === 0) {
                    ...
                    if (defaultLimitRaw && parseFloat(defaultLimitRaw) > 0) {
                        const totalSpent = Object.values(currentSpent).reduce((s, v) => s + v, 0);
                        if (totalSpent + priceTop + priceBottom + priceTote > parseFloat(defaultLimitRaw))
                            throw new Error(`ยอดรวมของเลข ${betNumber} เกินวงเงินเริ่มต้น`);
                    }
                }
            }
        }
        ...
        const billResult = yield client.query(`INSERT INTO bills (bill_ref, user_id, lotto_round_id, note, total_amount, bet_name, status, bill_lotto_draw) 
             VALUES ($1, $2, $3, $4, $5, $6, 'รอผล', $7) RETURNING id`, [billRef, userId, lottoRoundId, note, actualTotalAmount, lottoTypeDetails.name, billLottoDraw]);
        ...
        for (const entry of billEntries) {
            ...
            if (entry.betTypes === '19d')
                betTypeToSave = '2d';
            const entryResult = yield client.query(`INSERT INTO bill_entries (bill_id, bet_type, total) VALUES ($1, $2, $3) RETURNING id`, [newBillId, betTypeToSave, actualEntryTotal]);
            ...
            const processBetItems = (originalPrice, style, standardRate) => __awaiter(void 0, void 0, void 0, function* () {
                ...
                for (const betNumber of validBets) {
                    ...
                    yield client.query(`INSERT INTO bet_items (bill_entry_id, bet_number, price, bet_style, rate, payout_amount, baht_per) 
                         VALUES ($1, $2, $3, $4, $5, $6, $7)`, [newBillEntryId, betNumber, originalPrice, style, effectivePriceForPayout, finalPayoutAmount, payoutRate]);
                }
            });
    ...
    try {
        ...
        if (pendingBets && Array.isArray(pendingBets)) {
            for (const entry of pendingBets) {
                for (const betNumber of entry.bets) {
                    ...
                    if (entry.priceTop > 0) {
                        if (isThreeDigitMode) {
                            pendingMap[betNumber]['ตรง'] += entry.priceTop;
                        }
                        else {
                            pendingMap[betNumber]['บน'] += entry.priceTop;
                        }
                    }
        ...
        for (const betNumber in incomingTotals) {
            ...
            const applicableRules = rangeLimits.filter(r => r.range_start && r.range_end &&
                ...
                parseInt(betNumber, 10) <= parseInt(r.range_end, 10));
            let hasFailed = false;
            const check = (price, style) => {
                if (price <= 0 || hasFailed)
                    return;
                ...
                if (rule) {
                    ...
                    const currentSpent = (spentInDb[styleAliases[0]] || 0) + (spentInDb[styleAliases[1]] || 0) +
                        (spentInPending[styleAliases[0]] || 0) + (spentInPending[styleAliases[1]] || 0);
            ...
            if (totalRule) {
                // 1. ถ้ามีกฎ "ทั้งหมด" ที่เจาะจง ให้ใช้กฎนั้น
                ...
                // 2. ถ้าไม่มีกฎ "ทั้งหมด" ที่เจาะจง ให้ fallback ไปใช้กฎ Default
                const defaultLimitRaw = betNumber.length <= 2 ? roundLimits.limit_2d_amount : roundLimits.limit_3d_amount;
                if (defaultLimitRaw && parseFloat(defaultLimitRaw) > 0) {
    ...
    try {
        ...
        for (const limit of rangeLimits) {
            ...
            if (limit.range_start && limit.range_end && limit.max_amount) {
                ...
                yield client.query('INSERT INTO lotto_round_range_limits (lotto_round_id, range_start, range_end, max_amount, number_limit_types) VALUES ($1, $2, $3, $4, $5)', [id, limit.range_start, limit.range_end, limit.max_amount, limit.number_limit_types || 'ทั้งหมด'] // Koristimo fallback na 'sve'
                );
            }
        }
        yield client.query('COMMIT');
...
app.post('/api/bills/:billId/confirm', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const roundStatus = roundStatusResult.rows[0].status;
        if (roundStatus.includes('closed')) {
            throw new Error('ไม่สามารถยืนยันบิลได้ด้วยตนเอง เนื่องจากงวดนี้ปิดรับแล้ว (เซิฟเวอร์จะยืนยันบิลให้อัตโนมัติ แต่จะไม่สามารถยกเลิกหรือกระทำใดๆต่อบิลนี้ได้)');
        }
...
app.get("/api/prize-check/all-items", isAuthenticated, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { startDate, endDate, status, username, lottoName, lottoDate, billRef, derivedStatus } = req.query;
    if (!startDate || !endDate) {
        return res.status(400).json({ error: 'กรุณาระบุช่วงวันที่เริ่มต้นและสิ้นสุด' });
    }
    const queryParams = [];
    const whereConditions = [];
    let paramIndex = 1;
    // --- 1. การกรองช่วงเวลา (ตั้งแต่ - จนถึง) ---
    // ใช้คำสั่ง BETWEEN และบวกเวลา 23:59:59 ให้วันสิ้นสุด เพื่อให้ได้ข้อมูลครบถ้วนจนถึงวินาทีสุดท้ายของวันนั้น
    whereConditions.push(`b.created_at BETWEEN $${paramIndex++} AND $${paramIndex++}`);
    queryParams.push(startDate, `${endDate} 23:59:59`);
    // --- 2. การกรองข้อมูลพื้นฐาน (สิทธิ์การเข้าถึง) ---
    if (loggedInUser.role === 'owner' || loggedInUser.role === 'admin') {
        if (username && username !== 'all' && username !== '') {
            whereConditions.push(`u.username = $${paramIndex++}`);
            queryParams.push(username);
    ...
    // --- 4. Logic ตรวจรางวัลแบบครอบคลุม (หวยรัฐ + หวยหุ้น + เลขวิ่ง) ---
    // ใช้ JSONB @> เพื่อเช็คค่าใน Array และ strpos สำหรับเช็คเลขวิ่งในข้อความ
    const winningLogic = `(
        (be.bet_type IN ('3d', '6d') AND bi.bet_style = 'ตรง' AND lr.winning_numbers->'3top' @> to_jsonb(bi.bet_number::text)) OR
        (be.bet_type IN ('3d', '6d') AND bi.bet_style = 'โต๊ด' AND lr.winning_numbers->'3tote' @> to_jsonb(bi.bet_number::text)) OR
        (be.bet_type IN ('3d', '6d') AND bi.bet_style = 'ล่าง' AND lr.winning_numbers->'3bottom' @> to_jsonb(bi.bet_number::text)) OR
    ...
    // จัดการ Derived Status (ถูก/ไม่ถูกรางวัล)
    if (derivedStatus === 'ถูกรางวัล') {
        whereConditions.push(winningLogic);
        whereConditions.push(`lr.status IN ('closed', 'manual_closed')`);
        whereConditions.push(`bi.status = 'ยืนยัน'`);
    }
    else if (derivedStatus === 'ไม่ถูกรางวัล') {
    ...
    try {
        ...
        if (isNewFrontend) {
            // โหมดดึงผ่าน Pagination (ดึงข้อมูลได้ "ครบถ้วน" โดยการกดหน้าถัดไป)
            const page = parseInt(req.query.page, 10) || 1;
            const limit = parseInt(req.query.limit, 10);
            const offset = (page - 1) * limit;
            const countResult = yield db.query(`SELECT COUNT(bi.id) as "total" ${baseQuery}`, queryParams);
            const dataResult = yield db.query(`
                SELECT bi.*, be.bet_type, b.bill_ref, u.username, lr.name as "lottoName", lr.cutoff_datetime as "lottoDrawDate", lr.winning_numbers, lr.status as "lottoRoundStatus"
                ${baseQuery} 
                ORDER BY b.created_at DESC, bi.id ASC 
            ...
            // โหมดดึงครั้งเดียว (มี Safety Limit 2000 รายการ)
        ...
        res.status(500).json({ error: "เซิร์ฟเวอร์ขัดข้อง", details: err.message });
    }
}));
