...
  
  // State สำหรับ Pagination ของ "ตารางบิลล่าสุด"
...
  // State สำหรับ Pagination ของ "ตารางรายการถูกรางวัล"
...
    try {
      const [summaryResponse, recentBillsResponse, winningReportResponse] = await Promise.all([
        summaryRequest,
        recentBillsRequest,
...
    plugins: {
      legend: { display: legendDisplay, position: "top" as const, labels: { color: "#D1D5DB", font: { size: 12 } }, },
      title: { display: false },
      tooltip: {
        backgroundColor: "rgba(0, 0, 0, 0.8)",
        titleFont: { size: 14 },
        bodyFont: { size: 12 },
        callbacks: { label: (c: any) => `${c.label}: ${c.raw.toLocaleString()} ${ titleText.includes("ยอดขาย") ? "บาท" : "บิล" }`,},
      },
    },
    scales: {
...
      maintainAspectRatio: false,
      plugins: {
        legend: { display: legendDisplay, position: "top", labels: { color: "#D1D5DB", font: { size: 12 } }, },
        title: { display: false },
        tooltip: {
          backgroundColor: "rgba(0, 0, 0, 0.8)",
          titleFont: { size: 14 },
          bodyFont: { size: 12 },
          callbacks: { label: (c: any) => `${c.label}: ${c.raw.toLocaleString()} ${ titleText.includes("ยอดขาย") ? "บาท" : "บาท" }`, },
        },
      },
      scales: {
        x: { position: xPosition, beginAtZero: true, ticks: { color: "#9CA3AF" }, grid: { color: "rgba(156, 163, 175, 0.1)" }, title: { display: true, text: axis === "y" ? "ยอดแทง (บาท)" : "ตัวเลข", color: "#9CA3AF", font: { size: 14 }, }, },
        y: { ticks: { color: "#9CA3AF" }, grid: { display: false }, title: { display: true, text: axis === "x" ? "ยอดแทง (บาท)" : "ตัวเลข", color: "#9CA3AF", font: { size: 14 }, }, },
      },
    };
  };
                ...
                <div className="kpi-card">
                    ...
                    <div className="overflow-x-auto max-h-96 custom-scrollbar">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                {winningItems.length > 0 ? winningItems.map((group) => (
                                    <tr key={group.id} className="border-b border-gray-800 hover:bg-gray-800/50 whitespace-nowrap">
                                        ...
                                        <td className="p-3 whitespace-nowrap text-xs">
                                            {group.details.map((detail, index) => (<div key={index}>{getBetTypeName(detail.bet_type)} ({detail.bet_style})</div>))}
                                        </td>
                                  ...
                                  <tr><td colSpan={user?.role === 'admin' || user?.role === 'owner' ? 6 : 5} className="text-center py-10 text-gray-500 italic">ไม่มีรายการที่ถูกรางวัล</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
...
            <div className="kpi-card">
              <div className="flex justify-between items-center mb-4">
                ...
                {selectedBillIds.length > 0 && (
                    <button onClick={handleDeleteSelected} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:bg-gray-500 transition-colors text-sm font-bold flex items-center gap-2">
                        <TrashIcon className="h-4 w-4" /> ลบ {selectedBillIds.length} รายการที่เลือก
                    </button>
                )}
              </div>
