...
//     const groupedRounds = useMemo(() => allRounds.reduce((acc, round) => {
//         if (!acc[round.name]) acc[round.name] = [];
//         acc[round.name].push(round);
//         return acc;
//     }, {} as Record<string, LottoRound[]>), [allRounds]);
...
//     const processLottoNumberInput = (value: string, maxLength: number): string => {
//     // 1. กรองเอาเฉพาะอักขระที่อนุญาต (ตัวเลข, เว้นวรรค, คอมม่า)
//     const cleaned = value.replace(/[^0-9\s,]/g, '');

//     // 2. แยกแต่ละชุดตัวเลขออกจากกันด้วยเว้นวรรคหรือคอมม่า
//     const parts = cleaned.split(/[\s,]+/)
//         // 3. คัดกรองเอาเฉพาะชุดที่เป็นตัวเลข และมีความยาวไม่เกิน maxLength ที่กำหนด
//         .filter(part => /^\d+$/.test(part) && part.length <= maxLength);
    
//     // 4. นำชุดตัวเลขที่ถูกต้องกลับมารวมกันด้วยเว้นวรรคเดียว
//     return parts.join(' ');
// };
...
//         // 2. จัดระเบียบตัวคั่น: ทำให้ space หรือ comma ที่ติดกันหลายๆ ตัวกลายเป็น space เดียว
//         const normalized = cleaned.replace(/[\s,]+/g, ' ');

//         // 3. แยกแต่ละชุดตัวเลขออกมาเพื่อตรวจสอบชุดสุดท้าย
//         const parts = normalized.split(' ');
        
//         const lastPart = parts[parts.length - 1];

//         // 4. ตรวจสอบเงื่อนไข: ถ้าชุดสุดท้ายยาวเกิน 3 ตัว จะไม่ให้พิมพ์ต่อ
//         if (lastPart.length > 3) {
//             // คงค่าเดิมไว้ ไม่รับตัวอักษรที่ 4 เข้ามา
//             // โดยการนำส่วนก่อนหน้ามาต่อกับส่วนสุดท้ายที่ถูกตัดให้เหลือ 3 ตัว
//             const allButLast = parts.slice(0, -1);
//             const truncatedLastPart = lastPart.substring(0, 3);
//             processedValue = [...allButLast, truncatedLastPart].join(' ');
//         } else {
//             // ถ้ายังไม่เกิน 3 ตัว ก็ให้ใช้ค่าที่จัดระเบียบแล้วได้เลย
//             processedValue = normalized;
//         }
//     }
...
//     const handleDefaultLimitChange = (e: React.ChangeEvent<HTMLInputElement>) => {
...
//             setDefaultLimit(processedValue);
//         }
//     };
...
//     setter: React.Dispatch<React.SetStateAction<string>>
//     ) => {
//         // 1. ลบทุกอย่างที่ไม่ใช่ตัวเลข 0-9 ออกไป (รวมถึงจุดทศนิยม)
//         const onlyDigits = value.replace(/[^0-9]/g, '');

//         // 2. ถ้าค่าที่ได้เป็นค่าว่าง (เช่น ผู้ใช้ลบหมด) ให้ตั้งค่าเป็น "0"
//         if (onlyDigits === '') {
//             setter('0');
//             return;
//         }
        
//         // 3. แปลงเป็นตัวเลขแล้วกลับเป็นข้อความ เพื่อตัดศูนย์นำหน้าออก (เช่น "05" -> "5")
//         setter(String(Number(onlyDigits)));
//     };
...
//         <div className="bg-gray-900 text-white rounded-2xl shadow-lg overflow-hidden border border-gray-700/50">
//             <button onClick={() => setIsExpanded(prev => !prev)} className="w-full text-left p-4 flex justify-between items-center bg-gray-900/50 hover:bg-gray-700/30 focus:outline-none transition-colors">
//                 <h2 className="text-xl font-bold">จัดการงวดหวยและวงเงินที่อณุญาติให้ซื้อ</h2>
//                 <ChevronDownIcon className={`w-5 h-5 text-gray-400 transform transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
//             </button>
//             <div className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${isExpanded ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
//                 <div className="overflow-hidden">
...
//                                                                         {/* กลุ่มที่ 1: input สองตัวแรกจะอยู่ใน div นี้ และแสดงเป็นแถวเดียวกันเสมอ */}
//                                                                         <div className="flex w-full items-center gap-2">
//                                                                         <input 
//                                                                             type="text" 
//                                                                             placeholder="ตั้งแต่เลข" 
...
//                                                                         {/* กลุ่มที่ 3: ประเภทเลข และ ปุ่มลบ */}
//                                                                         <div className="flex w-full items-center sm:w-auto">
//                                                                         {startLen > 0 && <span className="text-xs text-gray-400 whitespace-nowrap">{getNumberTypeLabel(limit.range_start)}</span>}
//                                                                         {/* ใช้ ml-auto เพื่อดันปุ่มไปทางขวา */}
//                                                                         <button onClick={() => handleRemoveRangeLimit(index)} className="btn-icon-danger ml-auto">
//                                                                             <XCircleIcon className="h-5 w-5"/>
//                                                                         </button>
//                                                                         </div>
...
//                                                                             value={ex.exemption_type} 
//                                                                             onChange={e => handleExemptionChange(index, 'exemption_type', e.target.value)} 
//                                                                             className="input-field w-full sm:w-32 py-1.5" // ทำให้เต็มความกว้างบนจอเล็ก
//                                                                             disabled={!isOwner}
//                                                                         >
//                                                                             <option value="user">รายบุคคล</option>
//                                                                             <option value="role">ทั้งยศ</option>
